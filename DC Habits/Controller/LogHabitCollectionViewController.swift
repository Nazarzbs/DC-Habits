...
class LogHabitCollectionViewController: HabitCollectionViewController {
    ...
    override func configureCell(withItem cell: UICollectionViewListCell,
                                _ item: ViewModel.Item) {
        cell.configurationUpdateHandler = { cell, state in
            var content = UIListContentConfiguration.cell().updated(for: state)
            content.text = item.name
            content.directionalLayoutMargins = NSDirectionalEdgeInsets(top: 12, leading: 8, bottom: 12, trailing: 8)
            ...
            if state.isHighlighted {
                //Reduce the alpha of the tint color to 30% when highligted
